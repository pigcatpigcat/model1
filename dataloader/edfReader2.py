...
def main():
    ...
    # exclude_chs= ['Pz-Oz', 'horizontal', 'oro-nasal', 'submental', 'rectal', 'marker']
    # allchannels= ['EEG Fpz-Cz', 'EEG Pz-Oz', 'EOG horizontal', 'Resp oro-nasal', 'EMG submental', 'Temp rectal', 'Event marker']
    exclude_chs=[ 'Resp oro-nasal', 'Temp rectal', 'Event marker']
    # select_ch = 'EEG Fpz-Cz'
    ...
    select_ch = ["EEG Fpz-Cz","EOG horizontal","EMG submental"]
    ...
    for i in range(n):
        raw = read_raw_edf(psg_fnames[i],preload=True, stim_channel=None)#, preload=True, stim_channel=None
        print(raw.info)
        ...
        _, _, ann = zip(*reader_ann.records())#onset_sec, duration_sec, ann_char
        f.close()
        ...
        for a in ann[0]:
            ...
            if label != 5:
                ...
                print ("Remove onset:{}, duration:{}, label:{} ({})".format(
                    onset_sec, duration_sec, label, ann_str))
        print("remove index length:",len(remove_idx))
        labels = np.hstack(labels)

        print ("before remove unwanted: {}".format(np.arange(len(raw_ch_df)).shape))
        if len(remove_idx) > 0:
            remove_idx = np.hstack(remove_idx) #在水平方向上平铺
            select_idx = np.setdiff1d(np.arange(len(raw_ch_df)), remove_idx)#找到2个数组中集合元素的差异,在ar1中但不在ar2中的已排序的唯一值
        else:
            select_idx = np.arange(len(raw_ch_df))#一个有终点和起点的固定步长的排列，只有一个参数时，0开头
        ...
        # ss=EEG_Spectral_spatial_representation(x)
        # Save
...
def regularization(data: np.ndarray) -> np.ndarray:
    """
    :param data: the PSG data

    :return: PSG data after regularization
    """
    print(data.shape)
...
def exclude_channels(all_channels, target_channels):
